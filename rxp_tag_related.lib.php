...
function getRxpTagRelated($matches)
{
    $document_srl = $matches[1];
    $addon_info = $GLOBALS['_rxp_tag_related_addon_info_'];

    if (!$document_srl || !is_numeric($document_srl)) {
        return $matches[0];
    }

    $document_list = getRxpDocumentListWithinTag($document_srl);

    if (!$document_list) {
        return $matches[0];
    }

    Context::set('rxp_tag_related', $addon_info);
    Context::set('rxp_tag_related_document_list', $document_list);
...
function getRxpDocumentListWithinTag($document_srl)
{
    ...
    if ($oCacheHandler->isSupport()) {
        ...
        $cache_key = $oCacheHandler->getGroupKey('xemagazine-addon-rxp_tag_related', $object_key);
    ...
    // 동일 모듈(mid)의 글
    elseif ($addon_info->target == 'current_mid') {
    ...
    foreach ($output->data as $item) {
        // 원본 글 제거
        if ($item->document_srl == $document_srl) {
            continue;
        }

        // 최소 중첩 수 체크
        if ($addon_info->lowest_tag && $item->count < $addon_info->lowest_tag) {
            continue;
        }
